# Texas Communications Unit

The **Texas Communications Unit (TX-COMU)** is an independent 501(c)(3) nonprofit supporting, connecting, and strengthening the Communications Unit community across Texas.

We bring together professionals serving in incident communications, public-safety communications, emergency management, information technology, and related disciplines.

## Our Focus

- Strengthening relationships across the Communications Unit community
- Encouraging interoperability, readiness, and professional development
- Sharing knowledge, resources, and operational experience
- Supporting COML, COMT, COMC, ITSL, INCM, RADO, AUXCOMM, and related roles
- Developing practical, open-source tools for incident communications

## Featured Project

### ICT Branch Toolkit

An open-source toolkit for incident communications planning, including:

- ICS-205 development
- Current NIFOG channel resources
- Radio-site mapping
- Coverage visualization
- Frequency deconfliction
- Future TAK integration

ICT Branch Toolkit is being developed by TX-COMU for Communications Unit personnel and other public-safety organizations.

## Connect With Us

Visit [tx-comu.org](https://tx-comu.org) to learn more about TX-COMU, membership, programs, and current projects.

---

TX-COMU is an independent nonprofit community and is not the credentialing authority for Texas Communications Unit personnel.
